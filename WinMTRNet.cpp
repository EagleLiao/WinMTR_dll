...
//	TRACE_MSG("Threaad with TTL=" << current->ttl << " started.");
	...
	int				nDataLen = DEFAULT_PING_SIZE;	// wmtrnet->wmtrdlg->pingsize;	// 64
...
//			TRACE_MSG("TTL " << current->ttl << " reply TTL " << icmp_echo_reply->Options.Ttl << " Status " << icmp_echo_reply->Status << " Reply count " << dwReplyCount);
			...
			if(DEFAULT_INTERVAL * 1000 > icmp_echo_reply->RoundTripTime)		// 1
				Sleep(DEFAULT_INTERVAL * 1000 - icmp_echo_reply->RoundTripTime);
		}
...
//		TRACE_MSG("Start DnsResolverThread for new address " << addr << ". Old addr value was " << host[at].addr);
		...
		dnt->index = at;
		dnt->winmtr = this;
		if (DEFAULT_DNS) _beginthread(DnsResolverThread, 0, dnt);	// 1
	}
